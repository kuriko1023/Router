...
public:
  ...
  void
  handleRequest(const std::shared_ptr<ArpRequest> req);

  /**
   * send out enqueued packets corresponding to the ArpRequest
   */
  void 
  sendPendingPackets(const std::shared_ptr<ArpRequest> arp_req);

  /**
   * Checks if an IP->MAC mapping is in the cache. IP is in 
   * ! network byte order.
